...
The AL Orchestra system implements the **Analysis → Planning → Implementation → Review → Commit** cycle for Business Central AL development, adapted from the [GitHub Copilot Orchestra pattern](https://github.com/ShepAlderson/copilot-orchestra) with AL-specific enhancements.

> 💡 **New**: Before planning and implementation, use **MSY al-bc-analyzer** to research BC standard code and identify reusable structures. This prevents unnecessary custom development and avoids conflicts with standard logic.

## Architecture

```
┌──────────────────────────────────────────────────────────────────┐
│              MSY al-bc-analyzer (OPTIONAL PRE-STEP)              │
│  Researches BC Standard → Finds reusable structures → Outputs    │
│  technical analysis .md/.docx for dev team                       │
└────────────────────────┬─────────────────────────────────────────┘
                         ↓ (bc-analysis.md)
┌──────────────────────────────────────────────────────┐
│                  al-conductor                         │
...
### MSY al-bc-analyzer.agent.md
**BC Standard Code Analyst** that researches existing BC structures before any development starts.

**Usage**:
```markdown
Use MSY al-bc-analyzer mode

Analyze if BC standard already handles automatic payment reminders for overdue invoices
```

**Responsibilities**:
- Searches BC standard objects (Tables, Codeunits, Pages, Events) for reusable structures
- Evaluates whether requirements can be met using BC standard without custom code
- Identifies Integration Events and Business Events to subscribe to
- Verifies negative impacts on standard logic (posting engine, VAT, performance)
- Produces `.github/plans/{feature-slug}-bc-analysis.md` and `.docx`

**Tools**: `al-symbols-mcp`, `ms-dynamics-smb.al/al_download_source`, `read`, `edit`, `execute` (pandoc), `vscode/askQuestions`

**Model**: Claude Sonnet 4.6 (copilot)

**Output fed to**: `MSY al-conductor`, `MSY al-developer`, `MSY al-architect`
